...
function watchEmailChanges(auth) {
  ...
  gmail.users.watch(
    {
      userId: 'joniejacob1996@gmail.com',
      requestBody: {
        labelIds: ['INBOX'],
        topicName: 'projects/feisty-flames-387504/topics/RSVC_Pub_Sub',
      },
    },
    (err, res) => {
      if (err) {
  ...
  const pubsub = new PubSub({ projectId: 'feisty-flames-387504', keyFilename: './services/utils/mailer/keyFile.json' });
  // const pubsub = new PubSub()//{ projectId: 'feisty-flames-387504' });
  const subscriptionName = 'RSVC_Pub_Sub-sub';
